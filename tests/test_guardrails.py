...
# ---------------------------------------------------------------------------
# workflows — the CI definition must parse before it can gate anything
# ---------------------------------------------------------------------------


@pytest.fixture
def planted_workflow() -> Iterator[Path]:
    path = REPO_ROOT / ".github" / "workflows" / "_planted_probe.yml"
    try:
        yield path
    finally:
        path.unlink(missing_ok=True)
...
def test_workflows_catches_a_double_quoted_expression(
    planted_workflow: Path,
) -> None:
    """The exact bug that broke the R0.9 push, pinned.

    GitHub's expression language has no double-quoted string literal, so this is
    a *workflow parse* error: the whole file is rejected and every job silently
    fails to exist. Nothing inside the repository could catch it, because the
    suite that would have runs inside the workflow that failed to parse. Hence a
    pre-commit hook rather than only a test.
    """
    planted_workflow.write_text(
        "name: x\non: push\njobs:\n  a:\n    runs-on: ubuntu-latest\n"
        '    steps:\n      - run: echo ${{ join(needs.*.result, " ") }}\n',
        encoding="utf-8",
    )

    violations = workflows.check()

    assert any("double-quoted" in v.message for v in violations)


def test_workflows_allows_a_single_quoted_expression(
    planted_workflow: Path,
) -> None:
    planted_workflow.write_text(
        "name: x\non: push\njobs:\n  a:\n    runs-on: ubuntu-latest\n"
    ...
    assert workflows.check() == []


def test_workflows_catches_a_dependency_on_a_job_that_does_not_exist(
    planted_workflow: Path,
) -> None:
    """A typo here parses cleanly and then never gates anything."""
    planted_workflow.write_text(
        "name: x\non: push\njobs:\n"
        "  a:\n    runs-on: ubuntu-latest\n    steps:\n      - run: 'true'\n"
        "  b:\n    runs-on: ubuntu-latest\n    needs: [typo]\n"
    ...
    assert any("'typo'" in v.message for v in violations)


def test_workflows_accepts_a_string_needs(planted_workflow: Path) -> None:
    """`needs: a` is as valid as `needs: [a]` and must not be misread."""
    planted_workflow.write_text(
        "name: x\non: push\njobs:\n"
        "  a:\n    runs-on: ubuntu-latest\n    steps:\n      - run: 'true'\n"
        "  b:\n    runs-on: ubuntu-latest\n    needs: a\n"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "name: x\non: push\njobs:\n  a: 3\n", "not a mapping", id="job-scalar"
        ),
        pytest.param("a: [1,\n", "unparseable", id="broken-yaml"),
    ],
)
def test_workflows_rejects_a_malformed_workflow(
    planted_workflow: Path, source: str, expected: str
) -> None:
    planted_workflow.write_text(source, encoding="utf-8")

    violations = workflows.check()

    assert any(expected in v.message for v in violations), violations


def test_workflows_allows_a_reusable_workflow_job(planted_workflow: Path) -> None:
    """`uses:` at job level is a reusable workflow — no `runs-on` of its own."""
    planted_workflow.write_text(
...
def test_workflows_skips_an_undecodable_file(planted_workflow: Path) -> None:
    planted_workflow.write_bytes(b"\xfe\xff\x00 not valid utf-8 \xff")

    assert workflows.check() == []


def test_the_real_ci_anchor_job_depends_on_every_other_job() -> None:
    """Branch protection needs one required check, so it must cover them all.

    A job added above the anchor without being added to its `needs` is a job
    that runs and cannot block a merge — invisible once created.
    """
...
@pytest.mark.parametrize(
    "module",
    [ai_ban, network_isolation, read_only, no_secrets, workflows],
    ids=["ai_ban", "network_isolation", "read_only", "no_secrets", "workflows"],
)
