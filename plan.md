...
> **Structural finding (R2.1): a growing class of credential cannot be detected at all.** OAuth 2.0 client credentials — PayPal, Flutterwave v4, Coinbase CDP — are opaque strings with no vendor-published prefix, length or charset. There is nothing to write a rule *from*, and a rule matching "long opaque string, colon, long opaque string" would claim a large fraction of every base64 blob a scanner emits, which is a false-positive machine rather than a detection rule. keyreach's answer is `Provider.detectable = False`: the plugin is never a detection candidate, and the operator reaches it with `--provider <name>`, which the report records as an assertion rather than a verdict. **R2.2 confirmed this is not a payment-category problem**: Discord's bot token and Zoom's three-part Server-to-Server OAuth credential are equally undetectable, and neither is a payment API. It is a property of how modern credentials are designed — opaque strings, no vendor-published shape — so it is expected to become more common rather than less. §8 records which providers are affected; **R2.7** (generic bearer/JWT inspector) is the other half of the answer.
>
> **Fifth corollary (R2.7): "no vendor should claim every JWT" was never an argument that JWTs are undetectable — only that a *vendor-specific* rule should not claim them.** R2.2 kept Discord's community token pattern out and R2.5 kept Supabase's legacy keys undetectable on the identical reasoning: a rule matching three base64 segments would claim every JWT ever pasted at keyreach, misattributing it to one issuer among many that use the same open standard. `generic` resolves the tension rather than repeating it: a provider that names no vendor cannot misattribute anything, so it is exactly where that pattern belongs. `detect()` also requires the header and payload to actually decode as JSON — stronger evidence than shape alone — and the rule is the first in `detection_rules.yml` sourced to a standard (RFC 7515) rather than a vendor's documentation, because JWT's compact serialization is one.
>
> **Fourth corollary (R2.6): a write-only credential does not always get PyPI's treatment, even when its format is well documented.** Sentry's DSN is a URL-shaped credential as distinctive as PyPI's token, and Sentry states plainly that it "only allow[s] submission of new events and related event data; they do not allow read access to any information" — un-enumerable for the same reason a New Relic License key is: the one thing it authorises is a write, and confirming liveness read-only is impossible. PyPI's answer to that shape is a **detection rule with no plugin** (the second corollary below). Sentry gets **neither**, because PyPI's shape does not generalise when the same vendor also issues a second, better credential under the identical provider name: `keyreach/providers/sentry.py` already exists, built for Sentry's auth tokens, and a DSN detection rule would hand it a live, correctly-functioning write credential that the auth-token plugin would then report "invalid" for not being an auth token — a confident, wrong verdict, worse than the silence Firebase gets. The lesson: before reusing the "detection rule, no plugin" pattern, check whether the vendor's other credential already claims the provider name.
>
> **Third corollary (R2.5): some things that look like credentials are not credentials.** Undetectable and un-enumerable are both failures of keyreach's reach. Firebase is a failure of the *premise*. Google documents that "API keys for Firebase services do not need to be treated as secrets, and it's safe to include them in your code or configuration files", and — decisively — that "none of the Firebase-related APIs use an API key as authorization for calling the API. The API key passed with the API call is only used for identification of the Firebase project or app." A string that authorises nothing has no capability map, so there is nothing for `enumerate` to return and no severity that is not misleading. **keyreach ships no `firebase` provider**, and the reason is not that the work is hard: shipping one would tell a user their `AIza…` key is a Firebase secret when the vendor says the opposite, and `AIza…` is the *same* format as a Google Cloud API key, which genuinely does authorise Maps, Geocoding and billable calls — and which `keyreach/providers/google.py` already probes. The real Firebase exposure is a permissive Security Rule or an unrestricted Realtime Database, which is a property of the project rather than of the key, and confirming it means reading somebody's data. That is a scanner keyreach is not, and §11 forbids it. The lesson generalises past Firebase: **before writing a provider, check that the vendor treats the string as authorisation at all.**
>
> **Second corollary (R2.4): detection and enumeration fail independently, and PyPI is the case that separates them.** Everything above is about credentials keyreach cannot *recognise*. PyPI is the opposite: its format is documented, the rule is sound and re-verified, and keyreach still cannot say anything about what a live token reaches — because PyPI documents nine APIs and exactly one accepts an API token, `POST upload.pypi.org/legacy/`, which publishes a package. There is no authenticated read at all. Enumerating a PyPI token would mean performing a write to a package index, which §11 forbids outright and no argument about `read_only_post` can reach. So `pypi` ships as a **detection rule with no plugin**, which `tests/test_provider_contract.py` has always permitted ("the rule set may legitimately run ahead of the plugins") and which nothing had yet exercised. Detection alone still earns its place: the report names the vendor, the severity band and the rotation guide, which is most of what a disclosure needs. The two failures are recorded separately because the remedies differ — an undetectable credential is waiting on the vendor to publish a format, and an un-enumerable one is waiting on the vendor to publish a read.
>
> **Corollary (R2.3): a rule can also stop being verifiable after it ships.** keyreach carried a Mailgun rule from R0.5, sourced to Mailgun's authentication page. Re-reading that page for R2.3 found it now documents no key format at all, and neither does any other page Mailgun publishes. The rule still matched real legacy keys — it had not become *wrong* — but its cited source no longer supported it, and a rule nobody can re-verify is precisely what the provenance requirement above exists to prevent. So it was **withdrawn**, and `mailgun` joined the undetectable set. The distinction matters: PayPal, Discord and Zoom were never detectable, which is a fact about those vendors; Mailgun *was*, which is drift. Detection rules are therefore not write-once — they need re-verification on a schedule, which is what **R2.10**'s canary is for. The same pass found SendGrid's rule pinning two segment lengths that appear in no SendGrid document; those were relaxed to the shape SendGrid actually publishes.
...
**Shipped as of R2.7: thirty-two providers across nine categories.** `generic` joined in R2.7, opening the `generic` category — the first provider built around no single vendor. It decodes any JWT's claims offline (no signature check, `valid=False` on the same precedent as AWS's bare access key ID — see §5.2) and, when the operator supplies `TOKEN@URL`, probes that URL as a read-only liveness check. **This is also the first detection rule sourced to a standard (RFC 7515) rather than a vendor page** — the exact "three base64 segments" shape R2.2 and R2.5 kept out of Discord's and Supabase's own rules, because a provider claiming no vendor is the one place that pattern does not produce a false attribution.

**Shipped as of R2.6: thirty-one providers across eight categories.** `datadog`, `sentry`, `newrelic` and `grafana` joined in R2.6, opening the `monitoring` category. **Sentry's DSN was refused** on a fourth corollary in §5.2 — it is write-only, like a New Relic License key, but unlike PyPI it gets no detection rule either, because handing a live DSN to the `sentry` auth-token plugin would report it "invalid". **New Relic ships only its `NRAK-` User key**; the License key format is too generic to detect (a 40-character hex string, indistinguishable from a SHA-1 hash) and separately un-enumerable without sending real telemetry. **"Grafana" narrowed to Grafana Cloud's access policy tokens**, because a self-hosted instance's tokens have no fixed host to probe at all — a harder version of the gap that narrowed Redis to its control plane in R2.5.

**Shipped as of R2.5: twenty-seven providers across seven categories.** `mongodb`, `supabase`, `redis` and `pinecone` joined in R2.5, opening the `database` category. **Firebase was refused** on the third corollary in §5.2 — the vendor documents its API key as identifying rather than authorising — and "Redis" narrowed to Redis Cloud's control plane, because a Redis server credential is spoken over RESP rather than HTTP and cannot reach `ProbeContext` at all.

**Shipped as of R2.4: twenty-three providers across six categories.** `gitlab`, `bitbucket`, `npm` and `dockerhub` joined in R2.4, taking dev platforms to five. **PyPI ships as a detection rule with no plugin** — see the second corollary in §5.2 — and **npm's detection rule was withdrawn**, the second withdrawal in two items. Re-verifying the three devtools rules that already existed found one sound (`pypi`), one mis-cited (`gitlab`) and one unsupportable (`npm`), which is the clearest evidence yet that detection rules need re-verification on a schedule rather than at authoring time.

**Shipped as of R2.3: nineteen providers across six categories.** `sendgrid`, `mailgun`, `postmark`, `resend` and `mailchimp` joined in R2.3, opening the `email` category — the first item since R1.6 to ship every provider it named. Two of them are undetectable: `postmark` because neither of its token types has a published format, and **`mailgun` because keyreach withdrew a rule it had shipped since R0.5** (see the corollary in §5.2). Four of the five can be shown to *send* mail as the account without a message being sent, each from a different vendor statement — a scope, a refusal, a role, and the header a send endpoint requires.

**Shipped as of R2.2: fourteen providers across five categories.** `discord` and `zoom` joined in R2.2, both undetectable and reached with `--provider`. **PubNub is deferred** — see `ROADMAP.md` R2.2.
