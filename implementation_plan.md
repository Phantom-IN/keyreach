...
`keyreach/report/build.py` assembles a `Report` from an `EngineResult`; `keyreach/report/render.py` renders one to terminal text, JSON or Markdown. HTML landed in R2.9 — see §9.2 — through the same autoescaping loader configured here, which selects it by extension so nobody had to remember to turn it on later.
...
### 9.2 The fourth format, and one file that must open with no network (landed in R2.9)

`keyreach/report/templates/report.html.j2` and `render_html()` — the format §9.1 deferred. Same input, same ten `plan.md` §7 contents, same section order as `report.md.j2`; a different medium, not a different report.

- **A self-contained document is the actual requirement, not a nice-to-have.** A disclosure artifact opened from disk — attached to an email, saved from a bug-bounty portal — has to render correctly with no network available at all. Every style rule is therefore inlined in one `<style>` block; there is no `<link>`, `<script src>` or `<img src>` anywhere in the template, and `test_html_is_self_contained` asserts none crept in.
- **The autoescaping loader configured in R0.8 matches templates by filename suffix, and a compound suffix has to be spelled to match.** `select_autoescape(enabled_extensions=("html", "html.j2", "xml"), ...)` checks whether a template's name ends in `.html` **or** `.html.j2` — so the template is named `report.html.j2`, not `report_html.j2` or any other name that merely mentions "html". Get that filename wrong and autoescaping silently does not apply: every capability's `detail`/`evidence`/`poc` is vendor-sourced text that can legitimately contain `<`, `&`, or a proxy's raw HTML error page, and an unescaped render would let it become markup on the page. `test_html_escapes_a_capability_that_looks_like_markup` plants exactly that and asserts it comes out encoded.
- **A batch format is not free just because concatenation looks like it works for the other formats.** JSON batches as an array; Markdown batches by joining several sections with a separator, and both remain one walkable document either way a reader or a parser goes through it. HTML cannot: a document is one `<html>`/`<head>`/`<body>` tree, and several concatenated would parse in a browser as one mis-nested mess rather than fail loudly. Rather than invent an unspecified batch-index layout — this item's own roadmap line asks for "a disclosure-ready HTML **template**", singular — `_run_scan` (§12.1) refuses `--report html` combined with `--file`/stdin before a single key is probed.
- **Every test already parametrised over `ReportFormat` — the golden snapshots, the double-run determinism check, half of `test_render.py`'s renderer tests — extended to HTML by adding one enum member and one `SUFFIXES` entry**, with no test file needing new parametrisation logic. That is the same foresight paying off twice: `select_autoescape` was already HTML-aware from R0.8, and the test suite's format-agnostic parametrisation from the same era meant HTML inherited a full determinism and masking test suite on day one rather than needing one written from scratch.
- **Manual Jinja whitespace-control markers (`{%-`/`-%}`) fight `trim_blocks`/`lstrip_blocks` rather than complementing them.** The first draft used `-`-trimmed tags throughout and collapsed entire sections onto single run-on lines — valid HTML, unreadable in a diff. `report.md.j2` already answers this with plain `{% %}` tags and the environment's `trim_blocks=True, lstrip_blocks=True`, and matching that style rather than reaching for `-` markers produces the same clean line-per-element output the Markdown template has.
...
- **`--report html` refuses `--file`/stdin (roadmap R2.9).** JSON and Markdown batch by concatenation — a JSON array, or several Markdown sections back to back — because both are text formats a reader or a parser can walk regardless of how many findings are in the file. HTML cannot: a document is one `<html>`/`<head>`/`<body>` tree, and concatenating several would produce a page a browser mis-parses rather than a wrong-but-honest one. Rather than invent an unspecified batch-index layout, `_run_scan` rejects the combination with a message naming the alternatives — `--report md`/`--json` with `--file`, or one `keyreach` invocation per key for HTML.
